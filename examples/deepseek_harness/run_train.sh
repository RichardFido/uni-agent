#!/usr/bin/env bash
# Megatron + V1 async training for the blackbox deepseek_harness recipe.
#
# DeepSeek Harness (`dsh --profile headless`) runs *inside* the sandbox from a
# prebuilt tool image (mounted at /opt/dsh) and talks to the policy gateway
# through a reverse tunnel. This recipe uses the unified runner bridge:
#
#     uni_agent.framework.task_runner.run_task
#
# which resolves each sample's task from task_config_deepseek_harness.yaml
# (agent + sandbox defaults, incl. agent.tool_mode = full|minimal), deep-merges
# the sample values and the runtime model binding, injects the openyuanrong
# gateway reverse tunnel, and returns the reward via report_reward=True.
#
# Usage:
...
# deepseek_harness knobs (run_timeout/conda_env/tool_mode) and the tool-image
# mount are configured in TASK_CONFIG (task_config_deepseek_harness.yaml).
TASK_CONFIG="${TASK_CONFIG:-examples/deepseek_harness/task_config_deepseek_harness.yaml}"
...
# in the Task Config (task_config_deepseek_harness.yaml), not here.
...
SANDBOX_NAME_PREFIX="${SANDBOX_NAME_PREFIX:-dsh-}"

# ── Logging & checkpointing ──────────────────────────────────────────────
PROJECT_NAME="${PROJECT_NAME:-deepseek_harness}"
EXPERIMENT_NAME="${EXPERIMENT_NAME:-deepseek_harness_$(date +%Y%m%d_%H%M)}"
...
echo "=== DeepSeek Harness Blackbox Megatron Async Training ==="
